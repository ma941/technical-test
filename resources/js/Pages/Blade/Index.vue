    ...
    <wrapper>
        ...
        <div class="grid grid-cols-3 gap-2 mt-2">
            <div @click="navigateToBlade(blade.id)" v-for="blade in props.blades" class="border border-slate-500 py-4 px-12 rounded-md shadow cursor-pointer hover:bg-slate-500 hover:text-white hover:shadow-none">
                <p>Name: {{ blade.name }}</p>
                <p>Turbine: <span :class="blade.turbine ? '' : 'text-red-600'">{{ blade.turbine ? blade.turbine.name : 'N/A' }}</span></p>
                <p>Location: <span :class="blade.turbine ? '' : 'text-red-600'">{{ blade.turbine ? blade.turbine.wind_farm.location : 'N/A' }}</span></p>
